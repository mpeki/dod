...
export const CharacterOverview = (): React.JSX.Element => {
  ...
  const { activateCharHandler, fetchAllCharsHandler, errorCode, characters } = charContext;
  ...
  useEffect(() => {
    fetchAllCharsHandler().then();
  }, [fetchAllCharsHandler, activateCharHandler]);
  ...
  if (errorCode > 0) {
    ...
    return (
      <KeyboardShortcutProvider shortcuts={shortcuts}>
        {showCreateCharacter &&
          <AddCharacter fetchCharactersHandler={fetchAllCharsHandler} onConfirm={showCharacterHandler} />
        }
        <Stack direction={"row"}>
