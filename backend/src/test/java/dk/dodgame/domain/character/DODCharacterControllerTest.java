...
class DODCharacterControllerTest extends BaseControllerTest {
  ...
  void fetchAllCharactersShouldFetchList() throws Exception {
    List<CharacterDTO> resultList = Collections.singletonList(testBeing);
  ...
  void postBulkCreateShouldReturnArrayOfIDs() throws Exception {
    ...
    mockMvc
        .perform(MockMvcRequestBuilders.post("/api/char/bulk/create/3/human").contentType(MediaType.APPLICATION_JSON))
        .andDo(MockMvcResultHandlers.print())
        .andExpect(status().isOk())
