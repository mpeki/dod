...
public class CharacterTestUtil {
	...
	public static CharacterSkillDTO createRandomCharacterSkill(Category category, int fv) {
		String skillName = faker.job().keySkills();
		return CharacterSkillDTO.builder()
				.itemKey(ItemKey.toItemKey(skillName.toLowerCase()))
				.skill(
						SkillDTO.builder()
								.key(SkillKey.toSkillKey(skillName.toLowerCase()))
								.category(category)
								.group(Group.COMBAT)
								.build()
				).skillName(skillName)
